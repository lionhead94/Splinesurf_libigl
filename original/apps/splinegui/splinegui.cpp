  ...
  for (int i = 0; i < input.key_buttons.size(); i++) {
    ...
    if (key == gui_key('P')) {
      ...
      if (app.quadric_curve) {
        ...
        std::tie(left_q, right_q) = tuple_cat(insert_point(
        ...
        if (app.bezier_params.algorithm ==
          ...
          std::tie(left, right) = tuple_cat(insert_point(
              app.mesh, curve.tree, app.input().parameter_t));
        else
          std::tie(left, right) = tuple_cat(insert_point_spline(app.mesh,
...
void draw(const gui_input& input, void* data) {
  ...
  if (draw_button(widget, "Save test")) {
    for (auto i = 0; i < app.mesh.positions.size(); ++i) {
      std::cout << app.mesh.positions[i].x << app.mesh.positions[i].y
                << app.mesh.positions[i].z << std::endl;
    }
  ...
  //           app.mesh, app.bezier_params, app.bad_t, app.bad_points, false);
  //     }
  //   }
  ...
  //   //           app.bezier_params.subdivisions);
